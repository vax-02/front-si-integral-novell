...
export class DocentesComponent implements OnInit {
  ...
  loadAllSubjects(): void {
    this.subjectService.getSubjects(1, 200).subscribe({
      next: (data) => {
        this.allSubjects = data.subjects?.data ?? data.subjects ?? [];
      },
      error: () => {
  ...
  get modalTitle(): string {
    return this.editingDocenteId ? 'Editar Docente' : 'Agregar Docente';
  }
  get confirmSaveText(): string {
    return this.editingDocenteId ? 'Actualizar' : 'Guardar';
  }
  ...
  get availableSubjects(): any[] {
    ...
    const assigned = (this.docenteSeleccionado.subjects ?? []).map(
      (s: any) => s.id,
    );
  ...
  toggleStatus(docente: any): void {
    ...
    this.docenteService.toggleStatus(docente.id).subscribe({
      next: (res) => {
        docente.user.status = res.status;
  ...
  private syncDocenteInList(): void {
    ...
    if (idx !== -1)
      this.docentes[idx] = {
        ...this.docentes[idx],
        subjects: this.docenteSeleccionado.subjects,
      };
  }
  ...
  validarCampos(): boolean {
    ...
    if (!this.ci.trim()) {
      ...
      ok = false;
    } else if (!/^\d{5,12}$/.test(this.ci)) {
      this.errores['ci'] = 'El C.I. debe tener entre 5 y 12 dígitos';
      ok = false;
    }
    if (!this.correo.trim()) {
    ...
    if (this.celular && !/^\d{7,8}$/.test(this.celular)) {
      this.errores['celular'] = 'El celular debe tener 7 u 8 dígitos';
      ok = false;
    }
    if (!this.degreeId) {
